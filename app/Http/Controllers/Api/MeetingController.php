...
class MeetingController extends Controller
{
    ...
    //Vista de mis reuniones
    // GET /Mis Reuniones -> ver mis reuniones
    ...
    {   
        ...
        $reunionesT = titulacion::orderBy('fecha_inicio')->where('fecha_inicio', '>=', Carbon::today())->where('nombre_reservante', $nombre)->get();
        return view('reuniones', compact('reuniones', 'reunionesC', 'reunionesT'));
    }
    ...
    // Rutas de titulacion
    public function editTitulacion($id)
    {
        $reunion = titulacion::findOrFail($id);
        return view('edit_reunionTitulacion', compact('reunion'));
    }
    ...
    public function updateTitulacion(Request $request, $id)
    {
        ...
        $inicio = new \DateTime($request->fecha_inicio);
        $fin    = new \DateTime($request->fecha_fin);
        $reservante = titulacion::where('id', $id)->value('nombre_reservante');
        $motivo = $request->motivo_reunion;     
        $participantes = $request->participantes;

        // Validar traslapes, excluyendo la reunión actual
        $existeTraslape = titulacion::where('id', '!=', $id)
            ->where(function ($q) use ($inicio, $fin) {
                $q->where('fecha_inicio', '<', $fin)
                  ->where('fecha_fin', '>', $inicio);
            })->exists();
    ...
    //Borrar evento titulacion
    public function destroyTitulacion($id)
    {
        $reunion = titulacion::findOrFail($id);
        $reunion->delete();
        return redirect()->route('reuniones');
    ...
    // Rutas de titulacion
    public function indextitulacion(Request $request)
    {
        // FullCalendar suele mandar ?start=YYYY-MM-DD&end=YYYY-MM-DD
        $start = $request->query('start'); // opcional, para filtrar por rango
        $end   = $request->query('end');

        $query = titulacion::query();
    ...
    public function storetitulacion(Request $request)
    {
        $v = Validator::make($request->all(), [
            'nombre_reservante' => 'required|string|max:120',
            'motivo_reunion'    => 'required|string|max:255',
        ...
        // Validar traslapes:
        $existeTraslape = titulacion::where(function ($q) use ($inicio, $fin) {
            $q->where('fecha_inicio', '<', $fin)   // inicio existente antes de que termine la nueva
              ->where('fecha_fin', '>', $inicio);  // fin existente después de que empiece la nueva
        })->exists();
